...
func init() {
	...
	mid.RegInitItem3("DocImageProc", CreateEmpty, `{
...
var _ mid.GoFTLMiddleWare = (*DocImageProcType)(nil)
...
func (hdlr *DocImageProcType) ServeHTTP(www http.ResponseWriter, req *http.Request) {
	...
	if pn := lib.PathsMatchN(hdlr.Paths, req.URL.Path); pn >= 0 {
		if rw, ok := www.(*goftlmux.MidBuffer); ok {
			...
			trx := mid.GetTrx(rw)
			trx.PathMatched(1, "DocImageProc", hdlr.Paths, pn, req.URL.Path)
...
func (hdlr *DocImageProcType) getID() (id string) {
	key := hdlr.RedisID
