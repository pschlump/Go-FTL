...
func (c *Client) readPump() {
	...
	for {
		...
		if err != nil {
			if GorillaWebSocket.PJS_IsUnexpectedCloseError(err, GorillaWebSocket.CloseGoingAway) {
		...
		if dbFlag["echo-msg"] {
			fmt.Fprintf(os.Stderr, "readPump: Message [%s]\n", message)
		}
...
func (c *Client) clientBroadcast(message string) {
...
func (c *Client) writePump() {
	...
	for {
		...
		case <-ticker.C:
			c.conn.SetWriteDeadline(time.Now().Add(writeWait))
			if err := c.conn.WriteMessage(GorillaWebSocket.PingMessage, []byte{}); err != nil {
				return
			}
		}
	}
