...
func TestServeFile_DotDot(t *testing.T) {
	...
	for _, tt := range tests {
		req, err := http.ReadRequest(bufio.NewReader(strings.NewReader("GET " + tt.req + " HTTP/1.1\r\nHost: foo\r\n\r\n")))
		if err != nil {
...
func TestFSRedirect(t *testing.T) {
	defer afterTest(t)
	ts := httptest.NewServer(http.StripPrefix("/test", FileServer(http.Dir("."))))
	defer ts.Close()
...
func TestFileServerImplicitLeadingSlash(t *testing.T) {
	...
	ts := httptest.NewServer(http.StripPrefix("/bar/", FileServer(http.Dir(tempDir))))
	defer ts.Close()
...
func TestServeIndexHtml(t *testing.T) {
	...
	ts := httptest.NewServer(FileServer(http.Dir(".")))
	defer ts.Close()
...
func TestServerFileStatError(t *testing.T) {
	rec := httptest.NewRecorder()
	r, _ := http.NewRequest("GET", "http://foo/", nil)
	redirect := false
	name := "file.txt"
	fs := issue12991FS{}
...
func TestLinuxSendfileChild(*testing.T) {
	...
	mux := NewServeMux()
	mux.Handle("/", FileServer(http.Dir("testdata")))
...
func TestFileServerCleanPath(t *testing.T) {
	...
	for _, tt := range tests {
		...
		req, _ := http.NewRequest("GET", "http://foo.localhost"+tt.path, nil)
