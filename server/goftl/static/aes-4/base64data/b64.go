...
func (b Base64Data) Debug_dec(db bool, name string) {
	if db {
		log.Printf("%s: len=%d, 0x%x = %q = %v, %s", name, len(b), b, b.ConvToString(), b.Int32Array(), tr.LF(2))
	}
}

func (b Base64Data) Debug_hex(db bool, name string) {
	if db {
		log.Printf("%s: len=%d, 0x%x = %q = %x, %s", name, len(b), b, b.ConvToString(), b.Uint32Array(), tr.LF(2))
	}
}
