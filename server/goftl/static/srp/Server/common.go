...
// Copyright (C) 2015-2016 Philip Schlump
//
// License: MIT
//
...
// hash a number of byte strings and return the resulting hash as bigint
...
// Using a cryptographically strong random number generate pull out a 'n' sized random.
//var g_debug1 = true
//var g_db_pos = 0
//var g_ranv = []string{
func randbytes(n int) []byte {
	if g_debug1 {
		b := []byte(g_ranv[g_db_pos])
		fmt.Printf("Pulling random %d from table: %s\n", g_db_pos, b)
		g_db_pos++
		if g_db_pos >= len(g_ranv) {
			g_db_pos = 0
		}
		return b
	}
...
func randlong(bits int) *big.Int {
	if g_debug1 {
		...
		bb, ok := big.NewInt(0).SetString(b, 16)
		// gs.Salt, ok = big.NewInt(0).SetString(string(gs.Salt_s), 16)
		if !ok {
			panic("Invalid hex from debug - internal error")
		}
		return bb
	}
	n := bits / 8
...
func SessionKey(rawkey []byte, keylen int) []byte {
	...
	r := mathrand.Int31n(5000)
	sc := randbytes(keylen)
	return Pbkdf2(rawkey, sc, int(r), mac, keylen)
}

// Constant time string compare -- Repalce with subtle.ConstantTimeByteEq
